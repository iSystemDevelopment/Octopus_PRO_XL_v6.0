...
- **(firmware, critical) Manual / RND grid steps produced no sound.** The `SX_SUB_GRID_ROW` (sub 0x05) RX handler in `midi.cpp` guarded on `ps.sxPtr >= 10`, but the 10-byte App frame leaves only **9** bytes in `sxBuf` (the closing `F7` fires the parser before it is stored). Every App→device grid write — manual cell toggles **and** RND-generated patterns, melody **and** drums — was silently dropped, so `hwSeqData` never updated and the sequencer/drum voices never triggered. Factory **LOAD_PAT_S/D** sounded because it writes `hwSeqData` directly in firmware, bypassing the dead handler. Guard corrected to `>= 9`. **Reflash required.**
- **(firmware) Same-class off-by-one in the user name-blob RX guards** — `SX_SUB_USR_SOUND_NAME` `>=21→>=20` and `SX_SUB_USR_PAT_NAME` `>=20→>=19` (all three guards had used the on-wire byte count instead of the bytes-in-`sxBuf` count, which excludes `F7`). These paths are dormant today (App slot-name send path removed) but are now correct if the vault sender returns. The App→device handshake (connect `APP_SYNC_REQ` → `sendFullStateSync` burst + ordered done-marker) and device→App echoes were audited and are otherwise correct.
- **(App UI) Song / Pattern Manager bank dropdown showed "B…" for every step.** The per-step bank `<select>` (shared `buildSongEditor` — DSP Pattern Manager + MIDI Song mode) is 60 px wide, but its options were labelled "Bank A"…"Bank D" — clipped to the leading **"B" of "Bank"**, so the selected bank was never visible even though the model + wire value were always correct. Dropped the redundant "Bank " prefix → options now render **A / B / C / D**; added a hover tooltip. Affects both shells.
